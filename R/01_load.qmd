...
Data table 1 contains information on co-expression modules. Each column is a group (called module) of co-expressed genes. Each cell belonging to a column contains a gene that is part of this respective module. A total of 20 modules of co-expressed genes is defined. This data frame is therefore refered to as the coexpression data frame. The modules are hypothesised to represent biological pathways, but are not assigned to specific ones in the original work. Therefore, the names module 1-20 is retained in this project. The genes in this table relates to data table 3 as they are target molecules of upstream regulators described in the latter mentioned table. 

Data table 3 consists of data on several upstream regulators. Each row describes one upstream regulator molecule by characteristics such as the expression level, molecule type and target molecules as mentioned before. Data table 3 exists in two versions. One with data reflecting the RA patients before treatment and one after. The two dataframes are identical wrt. varaibles but not observations, that is the upstream regulators. There is some overlap in the observations but they also differ in which upstream molecules they describe. The data frame before treatment has a total of 849 upstream regulator whereas the dataframe after has 595 upstream regulators.   
