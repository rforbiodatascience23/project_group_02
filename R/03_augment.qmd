...
#### Augment the dataframe containing all observations before and after treatment

The function used below created a column that indicates weather a datapoing is significant or not.
Furthermore, a -log10 transformation of the p-values is added. 

```{r}
expr_before_aug <- significant_label(expr_before_aug, p_value_of_overlap_before, expr_log_ratio_before)
expr_after_aug <- significant_label(expr_after_aug, p_value_of_overlap_after, expr_log_ratio_after) 
```
...
target_intersect_aug <- target_intersect_aug |> 
  mutate(mol_change = n__target_after - n_target_before,
