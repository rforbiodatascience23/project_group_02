...
This augmentation to the target_data is performed before a tidy dataframe has been
obtained. This is done as the count of several values in a cell is easily intepreted
at the number of commas + 1. This way we do not have to iterate over several columns 
to count the number of values associated with similar variables. 
...
The chuck below can be deleted if we dont want the volcano plots as discused friday 24th Nov.
```{r}
write_tsv(expr_data_before_unfiltered,
          "../data/02_expr_data_before_unfiltered_clean.tsv.gz")

write_tsv(expr_data_after_unfiltered,
          "../data/02_expr_data_after_unfiltered_clean.tsv.gz")
```
