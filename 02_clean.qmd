...
split_target_before <- split_target_before |>
...
split_target_before  <- split_target_before |>
  separate("Target molecules in dataset (before)",
           str_c("Target molecule_",
                 seq(from = 1, to = split_target_before  |>
                       slice(1) |>
                       pull(Count))),
                 ",",
                 remove = TRUE,
...
pivot_before_treatment <- split_target_before |> 
  pivot_longer(cols = starts_with("Target"),
               names_to = "Target_n",
               values_to = "Target (before)")
pivot_after_treatment
```

```{r}
#| label: remove redundant variables and na (before)

tidy_pivot_before <- pivot_before_treatment |> 
  select(-"Target_n") |> 
  drop_na("Target (before)") |> 
  distinct()
```
...
p_value_removal_after <- regulators_before_clean |> 
...
split_target_after_clean <- count |> 
  ...
  pull(length_TM))),
           ...
           fill = "warn") |> 
  select(!"length_TM")
split_target_after_clean
```

```{r}
#| label: pivot longer 
pivot_after_treatment <- split_target_after_clean |> 
  pivot_longer(cols = starts_with("Target"),
               names_to = "Target_n",
               values_to = "Target (after)")
pivot_after_treatment
```
...
tidy_pivot_after <- pivot_after_treatment |> 
  select(-"Target_n") |> 
  drop_na("Target (after)") |> 
  distinct()
```
...
```{r}
regulators_data <- split_expression_before |> 
...
```{r}
expression_data <- tidy_pivot_before |> 
  full_join(tidy_pivot_after,
            by = "Upstream Regulator",
            relationship = "many-to-many") |> 
  distinct()
```
